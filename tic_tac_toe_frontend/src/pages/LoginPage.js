...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErr(e.detail ? e.detail[0]?.msg || 'Login failed.' : 'Login failed.');
    }
  };
  ...
  return (
    <div style={{ maxWidth: 350, margin: '48px auto', background: 'var(--bg-secondary)', padding: 24, borderRadius: 8, boxShadow: '0 2px 12px #0001' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <input type="email" required placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} autoFocus style={inputStyle} />
        <input type="password" required placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} style={inputStyle} />
        <button type="submit" className="theme-toggle" style={{ fontWeight: 600 }}>Login</button>
